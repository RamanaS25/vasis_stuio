...
export class LoginService {
  ...
  async login(user: any) {
    try {
      const { data, error } = await this.supabase
        .from('user_table')
        .select(
          'user_name, legal_name, initiated_name, grade, id, email, phone, voice_scale , is_banned, language, is_registered, is_admin, student_payment_history(session_id), student_groups(name, zoom_link, student_sessions(*)) '
        )
      ...
      if (!data.length) {
        ...
        if (data[0].is_banned) {
          return {
            success: false,
            error:
              data[0].language === 'English'
                ? 'Please complete your payment to access this level'
                : data[0].language === 'Portuguese'
                ? 'Por favor, complete o pagamento para acessar este nível'
                : data[0].language === 'Spanish'
                ? 'Por favor, complete el pago para acceder a este nivel'
                : 'Please complete your payment to access this level',
          };
        }
        ...
        this.user.payment_status = await this.check_if_paid(
          this.user.student_groups.student_sessions,
          this.user.student_payment_history
        );
        console.log('payment status', this.user.payment_status);

        if (
          this.user.payment_status.some(
            (x: { is_paid: string }) => x.is_paid === 'warning'
          )
        ) {
          this.logged_in = true;
          ...
          return {
            ...
            message:
              data[0].language === 'English'
                ? 'Please Complete your Payment for this level'
                : data[0].language === 'Portuguese'
                ? 'Por favor, complete o pagamento para acessar este nível'
                : data[0].language === 'Spanish'
                ? 'Por favor, complete el pago para acceder a este nivel'
          ...
          this.user.payment_status.some(
            (x: { is_paid: string }) => x.is_paid === 'banned'
          )
        ) {
          return {
            success: false,
  ...
  async check_if_paid(sessions: any, payment_history: any) {
    ...
    const payment_dates = sessions.filter(
      (session: any) =>
        session.week_num === 1 ||
        (session.week_num % 4 === 0 && session.week_num <= 20)
    );

    // Loop through each payment date
    payment_dates.forEach((session: any) => {
      // Check if the session has been paid for
      const isPaid = payment_history.some(
        (payment: any) => payment.session_id === session.id
      );
    ...
    const lastUnpaidSession = payment_status.find(
      (status: any) => status.is_paid === false
    );

    if (lastUnpaidSession) {
      const lastUnpaidDate = new Date(lastUnpaidSession.session_date);
      const daysDifference =
        (currentDate.getTime() - lastUnpaidDate.getTime()) /
